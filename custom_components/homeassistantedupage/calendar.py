...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    if coordinator.data.get("canteen_calendar_enabled", False):
        ...
        _LOGGER.debug("Canteen calendar skipped, no canteen menu data")
    ...
    def find_lesson_now_or_next_across_days(self) -> Optional[CalendarEvent]:
        ...
        for day, lessons in sorted(lessons_by_day.items(), key=lambda x: x[0]):
            ...
            if not future_lessons:
                continue
            candidate = min(future_lessons, key=lambda x: x.start_time)
            if next_lesson is None or candidate.start_time < next_lesson.start_time:
    ...
    def find_meal_now_or_next_across_days(self) -> Optional[CalendarEvent]:
        ...
        for day, meals in canteen_menu.items():
            for meal in meals:
                if day < today:
                    continue
                if (day == today and meal.served_from is None) or meal.served_from is None:
                    if day == today and meal.served_from is None:
                        meals_by_time.append((day, meal, now.time(), now.time()))
                        continue
                    meals_by_time.append((day, meal, None, None))
                    continue
                start_time = datetime.combine(day, meal.served_from.time())
                end_time = datetime.combine(day, meal.served_to.time()) if meal.served_to else None
                meals_by_time.append((day, meal, start_time, end_time))

        # A meal currently being served is the "event" right now.
        for day, meal, start_time, end_time in meals_by_time:
            if day == today and start_time is not None and end_time is not None:
        ...
        for day, meal, start_time, end_time in meals_by_time:
            if start_time is None:
                # no serving window: treat the start of the day as its slot
                candidate = datetime.combine(day, datetime.min.time())
            else:
