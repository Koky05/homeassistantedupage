...
async def _setup_services(hass: HomeAssistant, entry: ConfigEntry):
    """Register EduPage services bound to this config entry."""
        ...
        vol.Schema(
            {
                ...
                vol.Required("number"): vol.All(vol.Coerce(int), vol.Range(min=1, max=8)),
            }
        ),
    )
    hass.services.async_register(
        ...
        vol.Schema(
            {
                ...
                vol.Required("quantity"): vol.All(vol.Coerce(int), vol.Range(min=1, max=10)),
                vol.Required("quality"): vol.All(vol.Coerce(int), vol.Range(min=1, max=10)),
            }
        ),
    )
    hass.services.async_register(
